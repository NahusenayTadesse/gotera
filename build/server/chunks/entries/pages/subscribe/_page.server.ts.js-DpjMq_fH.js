import { m as db, F as addresses, q as plans, k as and, j as eq, t as subscriptions, p as subscribers, w as subscriberAddons, x as addons, a as asc, M as guestOrders, N as giftOrders } from '../../../chunks/db.js-BXYNtFGm.js';
import { h as getLocale } from '../../../chunks/runtime.js-CbeSlHLA.js';
import { l as lookupPostcode } from '../../../chunks/geocode.js-hCa1RR8a.js';
import { s as stripe } from '../../../chunks/stripe.js-DclyrhzZ.js';
import '../../../chunks/exports.js-BT-QlP_6.js';
import '../../../chunks/client.js-CWf6uOE8.js';
import { s as superValidate, a as setError, m as message } from '../../../chunks/client2.js-ZnyZ4fKl.js';
import { z as zod } from '../../../chunks/adapters.js-QNI96UbV.js';
import { l as loginSchema, a as addUser } from '../../../chunks/ZodSchema.js-ComZp2BR.js';
import { C as fail, B as redirect } from '../../../chunks/utils.js-BQt5v-8G.js';
import { o as object, n as number, s as string, y as email, c as boolean, a1 as number$1, b as string$1, w as record, x as array, _ as _enum, a4 as uuid } from '../../../chunks/access.js-BTJQW2Ke.js';
...
var checkoutSchema = object({
	...
	addonQuantities: record(string(), number$1().int().min(1).max(20)).default({}),
	...
	latitude: number$1().optional(),
	longitude: number$1().optional(),
...
async function resolveAddons(ids) {
	...
	const rows = (await db.select().from(addons).where(eq(addons.isActive, true))).filter((a) => ids.includes(a.id));
	return {
		rows,
...
var recurringFor = (interval) => interval === "bi_monthly" ? {
	interval: "month",
	interval_count: 2
} : {
	interval: "month",
...
*/
function addonLineItems(rows, recurring, quantities = {}) {
	return rows.map((a) => ({
		price_data: {
			currency: "gbp",
...
*/
var MAX_ADDON_QTY = 20;
function qtyOf(quantities, id) {
...
*/
function addonMetadata(rows, quantities = {}) {
...
async function oneTimeCheckout(opts) {
	...
	return (await stripe.checkout.sessions.create({
		...
		line_items: [{
			...
			quantity: opts.quantity
		}, ...addonLineItems(opts.addons, void 0, opts.addonQuantities)],
		...
		metadata: {
			giftOrderId,
			kind: opts.plan.kind,
			addonIds: addonMetadata(opts.addons, opts.addonQuantities),
...
var load = async ({ url }) => {
	const catalogue = await db.select().from(addons).where(eq(addons.isActive, true)).orderBy(asc(addons.sortOrder));
...
var actions = {
		...
		try {
			session = await stripe.checkout.sessions.create({
				...
				metadata: {
					...
					subscriptionId,
					addressId,
					addonIds: addonMetadata(chosenAddons, form.data.addonQuantities),
	...
	guestOrder: async ({ request, url }) => {
		...
		const [plan] = await db.select().from(plans).where(and(eq(plans.slug, form.data.plan), eq(plans.active, true)));
		if (!plan || plan.kind !== "order") return message(form, {
			type: "error",
...
//# sourceMappingURL=_page.server.ts.js-DpjMq_fH.js.map
