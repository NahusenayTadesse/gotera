import { a0 as head, a4 as ensure_array_like, a9 as escape_html, $ as attr, aa as attr_class, ab as stringify$1, T as derived } from '../../../chunks/server.js-CPNQ0GBv.js';
import { h as getLocale } from '../../../chunks/runtime.js-CbeSlHLA.js';
import { a as money, m as monthlyEquivalentPence } from '../../../chunks/format.js-DhQga0l2.js';
...
//#endregion
//#region src/lib/paraglide/messages/account_page_title.js
/** @typedef {import('../runtime.js').LocalizedString} LocalizedString */
/** @typedef {{}} Account_Page_TitleInputs */
var en_account_page_title = () => {
	return `Your account — GOTERA`;
};
var am_account_page_title = () => {
	return `መለያዎ — ጎቴራ`;
};
/**
* | output |
...
* | "Your account — GOTERA" |
*
* @param {Account_Page_TitleInputs} inputs
* @param {{ locale?: "en" | "am" }} options
* @returns {LocalizedString}
*/
var account_page_title = ((inputs = {}, options = {}) => {
	if ((options.locale ?? getLocale()) === "am") return am_account_page_title();
	return en_account_page_title();
});
//#endregion
//#region src/lib/paraglide/messages/account_cutoff_passed.js
/** @typedef {import('../runtime.js').LocalizedString} LocalizedString */
/** @typedef {{}} Account_Cutoff_PassedInputs */
var en_account_cutoff_passed = () => {
	return `Changes are closed for this delivery.`;
};
var am_account_cutoff_passed = () => {
	return `ለዚህ ማድረሻ ለውጦች ተዘግተዋል።`;
};
/**
* | output |
* | --- |
* | "Changes are closed for this delivery." |
*
* @param {Account_Cutoff_PassedInputs} inputs
* @param {{ locale?: "en" | "am" }} options
* @returns {LocalizedString}
*/
var account_cutoff_passed = ((inputs = {}, options = {}) => {
	if ((options.locale ?? getLocale()) === "am") return am_account_cutoff_passed();
	return en_account_cutoff_passed();
});
//#endregion
//#region src/lib/paraglide/messages/account_qty_decrease.js
/** @typedef {import('../runtime.js').LocalizedString} LocalizedString */
/** @typedef {{ name: NonNullable<unknown> }} Account_Qty_DecreaseInputs */
var en_account_qty_decrease = (i) => {
	return `Decrease ${i?.name} quantity`;
};
var am_account_qty_decrease = (i) => {
	return `የ${i?.name} ብዛት ቀንስ`;
};
/**
* | output |
* | --- |
...
*
* @param {Account_Qty_DecreaseInputs} inputs
* @param {{ locale?: "en" | "am" }} options
* @returns {LocalizedString}
*/
var account_qty_decrease = ((inputs, options = {}) => {
	if ((options.locale ?? getLocale()) === "am") return am_account_qty_decrease(inputs);
	return en_account_qty_decrease(inputs);
});
//#endregion
//#region src/lib/paraglide/messages/account_qty_increase.js
/** @typedef {import('../runtime.js').LocalizedString} LocalizedString */
/** @typedef {{ name: NonNullable<unknown> }} Account_Qty_IncreaseInputs */
var en_account_qty_increase = (i) => {
	return `Increase ${i?.name} quantity`;
};
var am_account_qty_increase = (i) => {
	return `የ${i?.name} ብዛት ጨምር`;
};
/**
* | output |
* | --- |
...
*
* @param {Account_Qty_IncreaseInputs} inputs
* @param {{ locale?: "en" | "am" }} options
* @returns {LocalizedString}
*/
var account_qty_increase = ((inputs, options = {}) => {
	if ((options.locale ?? getLocale()) === "am") return am_account_qty_increase(inputs);
	return en_account_qty_increase(inputs);
});
//#endregion
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const totalMonthlyPence = derived(() => data.subscriptions.filter((s) => s.status === "active" && !s.cancelAtPeriodEnd).reduce((sum, s) => sum + monthlyEquivalentPence(s.pricePence, s.interval), 0));
		const totalUnits = derived(() => data.subscriptions.reduce((sum, s) => sum + (s.quantity ?? 1), 0));
		const deliverableSubs = derived(() => data.subscriptions.filter((s) => s.nextDelivery && !s.nextDelivery.pastCutoff));
		let selectedSubId = "";
		const selectedDelivery = derived(() => deliverableSubs().find((s) => s.id === selectedSubId)?.nextDelivery ?? null);
		let pending = null;
		const basketCount = derived(() => data.addons.reduce((sum, a) => sum + qtyOf(a.id), 0));
		const basketPence = derived(() => data.addons.reduce((sum, a) => sum + a.pricePence * qtyOf(a.id), 0));
		const gbp = (pence) => money(pence);
		...
		head("8i5vi8", $$renderer, ($$renderer) => {
			$$renderer.title(($$renderer) => {
				$$renderer.push(`<title>${escape_html(account_page_title())}</title>`);
			});
		});
		if (data.notices?.length) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="notices svelte-8i5vi8"><!--[-->`);
			const each_array = ensure_array_like(data.notices);
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				let notice = each_array[$$index];
				$$renderer.push(`<div class="notice svelte-8i5vi8" role="status"><div><div class="notice-title svelte-8i5vi8">${escape_html(notice.title)}</div> `);
				if (notice.body) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<div class="notice-body svelte-8i5vi8">${escape_html(notice.body)}</div>`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--></div> <form method="POST" action="?/dismissNotice"><input type="hidden" name="id"${attr("value", notice.id)}/> <button type="submit" class="notice-close svelte-8i5vi8" aria-label="Dismiss">×</button></form></div>`);
			}
			$$renderer.push(`<!--]--></div>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> `);
		if (data.subscriptions.length === 0) {
			...
			for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
				...
				if (sub.nextDelivery) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<div class="delivery-date svelte-8i5vi8">${escape_html(sub.nextDelivery.dateLabel)}</div> <div class="delivery-detail svelte-8i5vi8">${escape_html(sub.nextDelivery.addressLine)}</div> `);
					if (sub.nextDelivery.pastCutoff) {
						$$renderer.push("<!--[0-->");
						$$renderer.push(`<span class="cutoff cutoff-closed svelte-8i5vi8">${escape_html(account_cutoff_passed())}</span>`);
					} else {
						$$renderer.push("<!--[-1-->");
						$$renderer.push(`<span class="cutoff svelte-8i5vi8">${escape_html(account_cutoff_label({ cutoff: sub.nextDelivery.cutoffLabel }))}</span>`);
					}
					$$renderer.push(`<!--]-->`);
				} else {
					$$renderer.push("<!--[-1-->");
				...
				if (sub.nextDelivery && !sub.nextDelivery.pastCutoff) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<form method="POST" action="?/skip"><input type="hidden" name="deliveryId"${attr("value", sub.nextDelivery.id)}/> <button type="submit" class="btn-ghost btn-full svelte-8i5vi8"${attr("disabled", pending === `skip:${sub.id}`, true)}>${escape_html(account_skip_delivery())}</button></form>`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--> `);
				if (sub.status === "paused") {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<form method="POST" action="?/resume"><input type="hidden" name="subscriptionId"${attr("value", sub.id)}/> <button type="submit" class="btn-ghost btn-full svelte-8i5vi8"${attr("disabled", pending === `resume:${sub.id}`, true)}>${escape_html(account_resume_plan())}</button></form>`);
				} else if (sub.status === "active" && !sub.cancelAtPeriodEnd) {
					$$renderer.push("<!--[1-->");
					$$renderer.push(`<form method="POST" action="?/pause"><input type="hidden" name="subscriptionId"${attr("value", sub.id)}/> <button type="submit" class="btn-ghost btn-full svelte-8i5vi8"${attr("disabled", pending === `pause:${sub.id}`, true)}>${escape_html(account_pause_plan())}</button></form>`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--> `);
				if (!sub.cancelAtPeriodEnd) {
					$$renderer.push("<!--[0-->");
				...
				$$renderer.push(`<!--]--></div></div> <dl class="stats-row svelte-8i5vi8"><div class="stat svelte-8i5vi8"><dt class="stat-label svelte-8i5vi8">${escape_html(account_stat_plan_label())}</dt> <dd class="stat-value svelte-8i5vi8">${escape_html(sub.planName)}</dd> <dd class="stat-sub svelte-8i5vi8">${escape_html(sub.packsLabel)}`);
				if (sub.quantity > 1) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`· ${escape_html(account_qty_label({ quantity: sub.quantity }))}`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--></dd></div> <div class="stat svelte-8i5vi8"><dt class="stat-label svelte-8i5vi8">${escape_html(account_stat_next_payment_label())}</dt> <dd class="stat-value svelte-8i5vi8">${escape_html(gbp(sub.pricePence))}</dd> <dd class="stat-sub svelte-8i5vi8">`);
				if (sub.quantity > 1) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`${escape_html(gbp(sub.unitPricePence))} × ${escape_html(sub.quantity)} ·`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]-->${escape_html(sub.nextPaymentDate ?? "—")}</dd></div> <div class="stat svelte-8i5vi8"><dt class="stat-label svelte-8i5vi8">${escape_html(account_stat_status_label())}</dt> <dd${attr_class("stat-value svelte-8i5vi8", void 0, { "green": sub.status === "active" })}>${escape_html(statusLabel()[sub.status] ?? sub.status)}</dd> <dd class="stat-sub svelte-8i5vi8">${escape_html(statusSub()[sub.status] ?? "")}</dd></div></dl></div>`);
			}
			...
			if (deliverableSubs().length === 0) {
				...
				if (deliverableSubs().length > 1) {
					...
					$$renderer.select({
						...
						$$renderer.push(`<!--[-->`);
						const each_array_2 = ensure_array_like(deliverableSubs());
				...
				$$renderer.push(`<!--]--> <form method="POST" action="?/addAddon"><input type="hidden" name="deliveryId"${attr("value", selectedDelivery()?.id ?? "")}/> <div class="addons-row svelte-8i5vi8"><!--[-->`);
				const each_array_3 = ensure_array_like(data.addons);
				for (let $$index_3 = 0, $$length = each_array_3.length; $$index_3 < $$length; $$index_3++) {
					let item = each_array_3[$$index_3];
					$$renderer.push(`<div class="addon svelte-8i5vi8"><div class="addon-img-placeholder svelte-8i5vi8"><span class="svelte-8i5vi8">${escape_html(item.name)}</span></div> <div class="addon-body svelte-8i5vi8"><div class="addon-head svelte-8i5vi8"><div class="addon-name svelte-8i5vi8">${escape_html(item.name)}</div> <div class="addon-price svelte-8i5vi8">${escape_html(gbp(item.pricePence))}</div></div> <div class="addon-desc svelte-8i5vi8">${escape_html(item.desc)}</div> <div class="addon-foot svelte-8i5vi8"><div class="qty svelte-8i5vi8"><button type="button" class="qty-btn svelte-8i5vi8"${attr("aria-label", account_qty_decrease({ name: item.name }))}>−</button> <div class="qty-n svelte-8i5vi8" aria-live="polite">${escape_html(qtyOf(item.id))}</div> <button type="button" class="qty-btn svelte-8i5vi8"${attr("aria-label", account_qty_increase({ name: item.name }))}>+</button></div> <input type="hidden"${attr("name", `qty_${stringify$1(item.id)}`)}${attr("value", qtyOf(item.id))}/></div></div></div>`);
				}
				$$renderer.push(`<!--]--></div> <div class="basket-bar svelte-8i5vi8"><div class="basket-total svelte-8i5vi8" aria-live="polite">`);
				if (basketCount() > 0) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`${escape_html(basketCount())} × ${escape_html(gbp(basketPence()))}`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--></div> <button type="submit" class="btn-outline svelte-8i5vi8"${attr("disabled", !selectedDelivery() || basketCount() === 0 || false, true)}>${escape_html(account_add_button())}`);
				if (basketCount() > 0) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`· ${escape_html(gbp(basketPence()))}`);
				} else $$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte.js-Dq1uOXoJ.js.map
