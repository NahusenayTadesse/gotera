	...
	$effect.pre(() => {
		if (editingId) {
			...
			if (row) {
				...
				$form.giftMessage = row.giftMessage ?? '';
...
<svelte:head>
	<title>One-Time & Gift Orders</title>
</svelte:head>
...
{#if editingId}
	<Card.Root class="mb-8 w-full lg:w-lg">
		...
		<Card.Content>
			<form method="POST" action="?/edit" use:enhance id="order-form" class="flex flex-col gap-4">
				<Errors allErrors={$allErrors} />
				<input type="hidden" name="id" bind:value={$form.id} />
				<InputComp label="Status" {form} {errors} type="select" name="status" items={statuses} required />
				<InputComp label="Buyer name" {form} {errors} type="text" name="buyerName" placeholder="Jane Doe" />
				<InputComp
					...
					required
				/>
				<InputComp label="Gift message" {form} {errors} type="textarea" name="giftMessage" placeholder="..." />
				<InputComp
