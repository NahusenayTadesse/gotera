<script lang="ts">
	...
	const getVar = (key: string, name: string) => {
		const el = chartCanvases[key];
		if (!el) return undefined;
		return getComputedStyle(el).getPropertyValue(name).trim() || undefined;
	};
	...
	const buildChartOptions = (key: string) => ({
		...
		plugins: {
			legend: {
				...
				labels: { color: getVar(key, '--foreground'), font: { size: 12 }, padding: 16 }
			},
			tooltip: {
				callbacks: {
					label: (ctx: any) =>
		...
		scales:
			type === 'bar' || type === 'line'
				? {
						x: {
							ticks: { color: getVar(key, '--muted-foreground'), font: { size: 11 } },
							grid: { color: getVar(key, '--border') }
						},
						y: {
							ticks: { color: getVar(key, '--muted-foreground'), font: { size: 11 } },
							grid: { color: getVar(key, '--border') },
...
{#if filtersOpen}
	<div class="mt-4" transition:fly={{ x: -100, duration: 300 }}>
		<Card class="w-full">
			...
			<CardContent>
				<div class="space-y-2 rounded-xl border border-border bg-background p-6 shadow-none">
					<!-- header row -->
